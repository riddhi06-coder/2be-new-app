...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* Section title */
    .section-title { background: #1a5276; color: #fff; padding: 5px 10px; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; margin-top: 18px; margin-bottom: 8px; border-radius: 3px; }
    ...
    table.data-table td.label { background: #eaf1fb; font-weight: 600; width: 38%; color: #1a5276; }
...
<div class="page">
    ...
    <div class="report-header clearfix">
        ...
        <p>Cesspool System Inspection Report</p>
    </div>
    ...
    <table class="data-table">
        ...
        <tr>
            <td class="label">Malfunction at Inspection</td>
        ...
        <tr>
            <td class="label">Surface Discharge</td>
            <td colspan="3">{{ $record->surface_discharge ?: '—' }}</td>
        </tr>
    </table>
    ...
    <table class="data-table">
        <tr>
            <td class="label">Accessible Lids</td>
            <td>{{ $record->accessible_lids ?: '—' }}</td>
            <td class="label">Lids Need Repair</td>
            <td>{{ $record->access_lid_repair ?: '—' }}</td>
        </tr>
        <tr>
            <td class="label">Water Level Depth</td>
            <td>{{ $record->cesspool_water_level_depth ?: '—' }}</td>
            <td class="label">Pumping Recommended</td>
            <td>{{ $record->pumping_recommended ?: '—' }}</td>
        </tr>
        <tr>
            <td class="label">Cesspool Pumped</td>
            <td>{{ $record->cesspool_pumped ?: '—' }}</td>
            <td class="label">Water Stream from House</td>
            <td>{{ $record->water_stream_from_house ?: '—' }}</td>
        </tr>
        <tr>
            <td class="label">Inlet Pipe Needs Repair</td>
            <td>{{ $record->inlet_pipe_needs_repair ?: '—' }}</td>
            <td class="label">Cesspool Composition</td>
            <td>{{ $record->cesspool_composition ?: '—' }}</td>
        </tr>
        <tr>
            <td class="label">Service Recommended</td>
            <td colspan="3">{{ $record->service_recommended ?: '—' }}</td>
        </tr>
        <tr>
    ...
    <p style="margin-top:14px; font-size:9px; color:#666; font-style:italic;">
        <strong>Disclaimer:</strong> The above information indicates the conditions of the cesspool system at the time of inspection.
        This is not a guarantee or warranty of future system performance.
    </p>

    <!-- Footer -->
    <div class="report-footer">
        2B Environmental &nbsp;|&nbsp; Cesspool Inspection Report &nbsp;|&nbsp; Record #{{ $record->id }} &nbsp;|&nbsp; {{ \Carbon\Carbon::now()->format('m/d/Y') }}
    </div>
