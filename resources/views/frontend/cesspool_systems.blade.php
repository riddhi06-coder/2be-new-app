...
<body>
    ...
    <script>
        ...
        function validateStep(step) {
            ...
            if (step === 0) {
                ...
                if (!window.cesspoolSignaturePad || window.cesspoolSignaturePad.isEmpty()) {
        ...
        document.getElementById('cesspoolForm').addEventListener('submit', function(e) {
            ...
            if (window.cesspoolSignaturePad && !window.cesspoolSignaturePad.isEmpty()) {
                document.getElementById('inspector_signature').value =
                    window.cesspoolSignaturePad.toDataURL('image/png');
            } else {
                document.getElementById('inspector_signature').value = '';
            }
    ...
    <script>
        (function () {
            ...
            window.cesspoolSignaturePad = new SignaturePad(canvas, {
            ...
            function resizeCanvas() {
                // Skip while canvas is hidden (parent step is display:none)
                if (canvas.offsetWidth === 0 || canvas.offsetHeight === 0) return;
                // Skip if size unchanged — avoid wiping the in-progress signature
                if (canvas.offsetWidth === lastW && canvas.offsetHeight === lastH) return;

                const ratio = Math.max(window.devicePixelRatio || 1, 1);
                const data  = window.cesspoolSignaturePad.toData();

                canvas.width  = canvas.offsetWidth  * ratio;
                canvas.height = canvas.offsetHeight * ratio;
                canvas.getContext('2d').scale(ratio, ratio);

                window.cesspoolSignaturePad.clear();
                if (data && data.length) window.cesspoolSignaturePad.fromData(data);

                lastW = canvas.offsetWidth;
                lastH = canvas.offsetHeight;
            }

            // Resize on window resize AND when canvas becomes visible after step nav
